...
// ─── ollama_create_model ─────────────────────────────────────────────────────
// Always available — used by the Grok→Ollama teaching workflow.

server.tool(
  'ollama_create_model',
  `Create a custom Ollama model from an existing base model plus a system prompt.
This is the final step in the Grok→Ollama teaching workflow: take the system prompt Grok generated,
bake it into a named Ollama model, and save it locally for testing or deployment.
The model can then be tested with ollama_generate.`,
  {
    name: z.string().describe('Name for the new model (e.g. "coding-expert", "writing-coach")'),
    base_model: z.string().describe('Base Ollama model to build from (e.g. "llama3.2", "mistral", "gemma3:1b")'),
    system_prompt: z.string().describe('The system prompt to bake into the model — use the output from grok_generate_prompt'),
    temperature: z.number().min(0).max(2).optional().describe('Optional temperature parameter (0–2)'),
  },
  async (args) => {
    log(`Creating model "${args.name}" from ${args.base_model}...`);
    writeStatus('creating', `Creating ${args.name} from ${args.base_model}`);
    try {
      // Build the Modelfile
      let modelfile = `FROM ${args.base_model}\nSYSTEM """\n${args.system_prompt}\n"""`;
      ...
      const res = await ollamaFetch('/api/create', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ model: args.name, modelfile, stream: false }),
      });
      ...
      log(`Created model "${args.name}" from ${args.base_model} — ${data.status}`);
      writeStatus('done', `Created ${args.name}`);

      return {
        content: [{
          type: 'text' as const,
          text: `Model "${args.name}" created successfully from ${args.base_model}.\n\nModelfile used:\n${modelfile}\n\nTest it with: ollama_generate(model="${args.name}", prompt="...")`,
        }],
      };
    } catch (err) {
      return {
        content: [{ type: 'text' as const, text: `Failed to create model: ${err instanceof Error ? err.message : String(err)}` }],
        isError: true,
      };
    }
  },
